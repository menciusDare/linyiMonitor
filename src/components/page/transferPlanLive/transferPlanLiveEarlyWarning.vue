...
export default {
  ...
  data() {
    return {
      ...
      search: {
        regionCode1: sessionStorage.getItem("regionCode"),
  ...
  methods: {
    ...
    initPage() {
      var _this = this;
      asyncLoadBaiduMapJsApi(_this.AppConfig.appInfo.baiDuMapAk, 1)
        .then(() => {
          _this.initBMap();
        })
        .catch((err) => {
    ...
    getBoundary() {
      ...
      // /this.level = 3
      bdary.get(this.regionName, function (rs) {
        //获取行政区域
    ...
    searchMap() {
      ...
      if (this.search.regionName == this.AppConfig.appInfo.regionName) {
    ...
    getEntMaps(regionCode, regionName, bssw, bsne, entType, userRegionCode) {
      ...
      var _this = this;
      _this.$axios
        .post("/api/regulatory/tEnterpriseInfo/getEntOnMap",  {
          startDate: _this.value1[0],
          endDate: _this.value1[1],
          typeId: _this.dictDetailCode,
          regionCode: _this.getAdcd(userRegionCode), // 当前
        ...
        .then(function (res) {
          if (res.code == 0) {
            ...
            _this.addEntMaker(res.data, 1); //向地图中添加统计点
          }
        })
        .catch((error) => {
          // catch 指请求出错的处理
    ...
    getRegionCountData() {
      var _this = this;
      _this.$axios
        .post("/api/regulatory/selectTranPlanWarning", {
          startDate: _this.value1[0],
          endDate: _this.value1[1],
          typeId: _this.dictDetailCode,
          //  "parentRegionCode": parentRegionCode,// 父级点位code
          regionCode: _this.getAdcd(_this.search.regionCode), // 当前
          transportEnterpriseName: _this.searchValue, //搜索内容
          // "userRegionCode": userRegionCode
        })
        .then(function (res) {
          _this.addRegionMarker(res.data);
          ;
          if (_this.search.regionName || _this.search.regionName== this.AppConfig.appInfo.regionName) {
            _this.getBoundary1(_this.search.regionName);
          } else {
            let regions = JSON.parse(sessionStorage.getItem("regions"));
            regions.forEach((element) => {
              _this.getBoundary1(
                this.AppConfig.appInfo.regionName + element.name
              );
            });
          }
    ...
    getStatistics() {
      var _this = this;
      _this.$axios
        .get("/api/regulatory/getTEnterpriseTransportPlanWarningStatistics")
        .then(function (res) {
          if (res.code == 0) {
    ...
    addEntMaker(dataList, type) {
      if (type === 0) {
        var mycfIcon = new BMap.Icon(
      ...
      } else {for (var i = 0; i < dataList.length; i++) {
        //遍历
        // 过滤0数据
        var infoA = dataList[i].lng;
        var infoB = dataList[i].lat;
        // var myIcon = new BMap.Icon("../../../../static/css/images/map/白色背景1@2x.png", new BMap.Size(200,65));
        let myIcon = new BMap.Icon(
          "../../../../static/css/images/map/barbg.png",
          new BMap.Size(20, 50)
        );
        // 添加区域信息点位
        ...
        if (dataList[i].currentAmount > 0) {
          var labelContext = ` <div class="map-sign level-03" style="">
                                    <div class="map-sign-bar" style="height: 30px;">
                                        <div class="map-sign-num">${dataList[i].currentAmount}kg</div>
                                        <div class="map-sign-text">${dataList[i].name}</div>
                                    </div>
                                </div>
                            `;
        ...
        //         str += entCount[j].entKind + ':&nbsp;' + entCount[j].count + '吨</br>'
        //     }
        //     var sty = "#fff"
        //     if(dataList[i].regionName=="HW08"){
        //         sty = "red";
        //     }
        //    labelContext = '<div><em style=" display:block; padding-top:2px;padding-left:15px;margin:0px auto;font-size:16px;line-height:36px;position: relative;">'+str+'</em><div style="position: absolute;top:-66px;display: block;padding-left: 6px;padding-top: 5px;padding-right: 8px;height: 50px;width:135px;border: 2px solid #cf9236;margin-top: 16px;margin-left: -2px;font-size: 25px;border-radius: 5px;background-color: #ffb700;color: '+sty+';text-align:center ">'+ dataList[i].regionName +'</div></div>';
        // }else{
        //     labelContext = '<div><em style=" display:block; padding-top:2px;padding-left:17px;margin:0px auto;font-size:20px;line-height:36px;position: relative;">'+'无数据</em><div style="position: absolute;top:-66px;display: block;padding-left: 6px;padding-top: 5px;padding-right: 8px;height: 50px;width:135px;border: 2px solid #cf9236;margin-top: 16px;margin-left: -2px;font-size: 25px;border-radius: 5px;background-color: #ffb700;color: #fff;text-align:center ">' + dataList[i].regionName + '</div></div>';
        // }
      }
      }
    },
    ...
    entInfoLook(entInfo) {
      ...
      // 查询企业详情
      _this.$axios
        .get("/api/regulatory/index/getEntMapInfo", {
          params: {
            entId: entInfo.entId,
        ...
        .then(function (res) {
          if (res.code == 0) {
            ...
            _this.registeredAddress =
              res.data.registeredAddress.length > 12
                ? res.data.registeredAddress.substring(0, 12) + "..."
                : res.data.registeredAddress;
            if (res.data && res.data.realTimeStockVo) {
              _this.containeGroupVoList =
                res.data.realTimeStockVo.containeGroupVoList;
              _this.stockScale = res.data.realTimeStockVo.stockScale;
              _this.groupStorageCurrentSum =
                res.data.realTimeStockVo.currentStockNum;
            }
          }
        })
        .catch((error) => {
        ...
        .then(function (res) {
          if (res.code == 0) {
            ...
            sessionStorage.setItem(
              "regions",
              JSON.stringify(res.data[0].children)
            );
    ...
    addRegionMarker(dataList) {
      ...
      for (var i = 0; i < dataList.length; i++) {
        ...
        if (dataList[i].sumNuber > 0) {
          var labelContext = ` <div class="map-sign level-03" style="">
                                    <div class="map-sign-bar" style="height: 30px;">
                                        <div class="map-sign-num">${dataList[i].sumNuber?(dataList[i].sumNuber/1000).toFixed(2):'--'}吨</div>
