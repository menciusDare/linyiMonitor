...
//                            _this.enterpriseAndData.enterprise.EnterpriseEchars.series[0].data = res.data;
            ...
            searchData(){
                ...
                if (zoom >= 1 && zoom <= 16 && self.level != 3) {
                    if (zoom <= 14) {// 区县
                        if(self.typeSelected === 'qiye'){
                ...
                if(_this.typeSelected === 'qiye'){
                    for (var i = 0; i < dataList.length; i++) {//遍历
                        if (dataList[i].list.length != 0) {
                            ...
                            // this.map.addOverlay(new BMap.Marker(new BMap.Point(infoA, infoB)));
            ...
            regionInfoMakerClick(regionInfo){
                ...
                if (thisLevel == 2 || thisLevel == 1) {// 市
                    ...
                    if(regionInfo.regionCode===371302){
                        self.map.centerAndZoom(new BMap.Point(self.lanshanLng,self.lanshanLat), 13);
                    }else {
                        self.map.centerAndZoom(new BMap.Point(regionInfo.lng, regionInfo.lat), 13);
                    }
                    // 查询企业点位数据
                    // console.log("显示")
                    if(self.typeSelected === 'qiye'){
