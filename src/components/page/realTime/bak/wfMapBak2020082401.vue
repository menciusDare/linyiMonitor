                ...
                <div class="" v-for= "(item, index) in enterpriseAndData" :key="index">
                    <div v-if="item.key === typeSelected" class="EnterpriseMain">
                        <div class="detailTop" >
                            ...
                            <div v-if="item.key === 'shuju'&typeSelectedShuJu === 'enter'" class="count">{{count}}吨</div>
                    ...
                    dataList.map(item =>{
                        ...
                        if(isAlarm === true){
                            ...
                            var labelText = `<div class= "IConMain ">
                                <div class="iconLine">
                                    <span class=${spanClass}>${standardValue}</span>
                                    <span> ${name} </span>
                                </div>
                                <div class="sanjiao"></div>
                            </div>`
                ...
                if(this.typeSelected === 'shuju') {
                    ...
                    dataList.map(item => {
                        ...
                        if(yearAmount!=0){
                            ...
                            var token = Math.floor(count/100)*10;
                            if(token>25){
                                height = 100+token;
                            }else{
                                height = 50+token;
                            ...
                            let labelContext = '<div class="map-sign level-02" style="">'
                                +'<div class="map-sign-bar" style="height: '+height+'px;">'
                                +'<div class="map-sign-num">'+((yearAmount?yearAmount:0)/1000).toFixed(2)+'吨</div>'
...
//                $(".EnterpriseMain").fadeOut()
            ...
            searchRegionChange(){
                ...
                customOverlay.clearMap("regionBoundaryOver",this);
                if(name == this.AppConfig.appInfo.regionName){
                    this.drawRegionBoundary();
                }else{
        ...
        watch: {
            typeSelected: function(){
                if(this.typeSelected=='qiye'){
                    ...
                    this.clearOver();
                    this.getRegionWfCountData('', '', 3, this.initBssw, this.initBsne, this.dicType,this.dataCountSource, this.search.regionCode);// 请求统计数据
                    this.getRegionWfPieChartCountData('', '', 3, this.initBssw, this.initBsne, this.dicType,this.dataCountSource, this.search.regionCode);// 请求统计数据
