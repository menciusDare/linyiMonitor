<template>
    ...
    <div class="main">
        ...
        <div class="panel">
            <div class="panel-bd">
                ...
                <div>
                    <ul class="filter-box">
                        ...
                        <li class="filter-item">
                            <div class="filter-label">提交季度：</div>
                            <div class="filter-con">
                                <el-select v-model="search.declareQuarter">
                                    <el-option
                                            v-for="item in declareQuarterList"
                                            :key="item.value"
                                            :label="item.label"
                                            :value="item.value">
                                    </el-option>
                    ...
                    <span class="el-input__prefix"><i class="el-input__icon el-icon-search"></i>
                                    ...
                                    <el-input type="text" autocomplete="off" placeholder="搜索企业名称"
                        ...
                        <li class="filter-item">
                            <div class="filter-btn">
                                <!-- <button type="button" @click="searchEntManagementPlanReport" class="button-type">
                                     <span>查询</span></button>-->
                                <el-button type="button" size="medium" @click="searchEntManagementPlanReport"
                ...
                <div class="total-number" style="margin-left: 3px;">
                    <!-- <div class="total-name">全部企业{{count.count}}家</div> -->
                    <div class="total-text">已提交企业<span class="num" style="color:green">{{count.commitCount}}</span>家
                    </div>
                    <div class="total-text is-offline">未提交企业<span class="num"
                                                                  style="color:red">{{count.notCount}}</span>家
                    </div>
                </div>
                <table class="table-default">
                    ...
                    <tr v-for="(item,index) in dataList" :key="index">
                        ...
                        <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 10%">
                            <div v-if="item.entKind == 0">
                                运营企业
                            </div>
                            <div v-else-if="item.entKind == 1">
                                产废企业
                            </div>
                            <div v-else-if="item.entKind == 2">
                                经营企业
                            </div>
                            <div v-else-if="item.entKind == 3">
                                收集企业
                            </div>
                            <div v-else-if="item.entKind == 4">
                                运输企业
                            </div>
                        </td>
                        <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 8%"
                            :title="item.regionName">{{item.regionName}}
                        </td>
                        <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 8%"
                            :title="item.categoryName">{{item.categoryName}}
                        </td>
                        <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 10%"
                            :title="item.contacts">{{item.contacts}}
                        </td>
                        <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 8%"
                            :title="item.contactsPhone">{{item.contactsPhone}}
                        </td>
                        <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 10%"
                        >
                            <div v-if="item.status == -1" class="text-state state-anomaly">审核拒绝</div>
                            <div v-if="item.status == 0" class="text-state untreated">已提交未审核</div>
                            <div v-if="item.status == 1" class="text-state">审核通过</div>
                            <div v-if="item.status == 2" class="text-state state-off">未提交</div>
                        </td>
                        <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 10%"
                            :title="item.commitTime">
                            <div v-if="item.commitTime">{{item.commitTime}}</div>
                            <div v-else>--</div>
                        </td>
                        <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 10%">
                            <div class="btn-opreate">
                                <a
                                        v-if="item.commitTime"
                                        @click="lookEntMonthPeportNew(item.entId,item.entName)"
                                        href="javascript:;"
            ...
            <el-dialog
                ...
                <div style="margin-top:10px">
                    <el-tabs :tab-position="tabPosition" :stretch="true" v-model="productionName"
                    ...
                    <div style="padding-bottom: 12px;height: 236px;overflow: auto;">
                        <table class="mytable-default">
                            ...
                            <tr v-for="(item,index) in entMonthOrYearReportInfo.reportEntProductionVoList" :key="index">
                                ...
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 15%"
                                ...
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 20%"
                                    :title="item.harmfulName">{{item.harmfulName}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 15%"
                                    :title="item.productionNumber">
                                    {{(item.productionNumber?item.productionNumber:0).toFixed(2)}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 15%"
                                    :title="item.handleType">{{item.handleType}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 15%"
                                    :title="item.handleNumber">{{(item.handleNumber?item.handleNumber:0).toFixed(2)}}
                                </td>
                            </tr>
                            <tr v-if="entMonthOrYearReportInfo.reportEntProductionVoList">
                                ...
                                <td align="center">
                                    {{(entDialogCountInfo.handleNumber?entDialogCountInfo.handleNumber:0).toFixed(2)}}
                                </td>
                            </tr>
                            </tbody>
                        </table>
                    ...
                    <div style="padding-bottom: 12px;height: 236px;overflow: auto;">
                        <table class="mytable-default">
                            ...
                            <tr v-for="(item,index) in entMonthOrYearReportInfo.reportEntEntrusts" :key="index">
                                ...
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 15%"
                                    :title="item.client">{{item.client}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 10%"
                                    :title="item.location">{{item.location}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 15%"
                                    :title="item.license">{{item.license}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 15%"
                                    :title="item.handleType">{{item.handleType}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 10%"
                                    :title="item.entrustNumber">{{(item.entrustNumber?item.entrustNumber:0).toFixed(2)}}
                                </td>
                            </tr>
                            <tr v-if="entMonthOrYearReportInfo.reportEntEntrusts">
                                ...
                                <td align="center">
                                    {{(entDialogCountInfo.entrustNumber?entDialogCountInfo.entrustNumber:0).toFixed(2)}}
                                </td>
                            </tr>
                            </tbody>
                        </table>
                    ...
                    <div style="padding-bottom: 12px;height: 236px;overflow: auto;">
                        <table class="mytable-default">
                            ...
                            <tr>
                                ...
                                <th style="text-align:center;width: 20%">危险废物名称</th>
                                <th style="text-align:center;width: 20%">上月遗留贮存量(吨)</th>
                            ...
                            <tr v-for="(item,index) in entMonthOrYearReportInfo.reportEntStorageVos" :key="index">
                                ...
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 20%"
                                    :title="item.legacyMonth">{{(item.legacyMonth?item.legacyMonth:0).toFixed(2)}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 20%"
                                    :title="item.lastMonth">{{(item.lastMonth?item.lastMonth:0).toFixed(2)}}
                                </td>
                                <td style="text-align:center;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;width: 20%"
                                    :title="item.thisMonth">{{(item.thisMonth?item.thisMonth:0).toFixed(2)}}
                                </td>
                            </tr>
                            <tr v-if="entMonthOrYearReportInfo.reportEntStorageVos">
                                ...
                                <td align="center">
                                    {{(entDialogCountInfo.legacyMonth?entDialogCountInfo.legacyMonth:0).toFixed(2)}}
                                </td>
                                <td align="center">
                                    {{(entDialogCountInfo.lastMonth?entDialogCountInfo.lastMonth:0).toFixed(2)}}
                                </td>
                                <td align="center">
                                    {{(entDialogCountInfo.thisMonth?entDialogCountInfo.thisMonth:0).toFixed(2)}}
                                </td>
                            </tr>
                            </tbody>
                        </table>
        ...
        >
            ...
            <div>
                <el-tabs :tab-position="tabPosition" v-model="productionNameQuarter" @tab-click="yearClick">
                    <el-tab-pane
                            v-for="item in declareQuarterList"
                            :key="item.label"
                            :label="item.label"
            ...
            >
                        ...
                        >
                            <el-card>
                                <el-row>
                                    ...
                                    <el-col :span="18">{{item.auditUserName}}</el-col>
                                </el-row>
                                <br/>
                                <el-row>
                                    <el-col :span="4">
                                    ...
                                    <el-col :span="18">{{item.auditStatus == 1 ? "通过" : "拒绝"}}</el-col>
                                </el-row>
                                <br/>
                                ...
                                <el-row>
                                    ...
                                    <el-col :span="18" style="letter-spacing:1px;line-height: 22px">{{item.auditContent}}
                                    </el-col>
                                </el-row>
                            </el-card>
                        </el-timeline-item>
                ...
                <div v-if="auditStatus != 1 && auditStatus!=-1">
                    ...
                    <el-form-item prop="remark" label="请填写审核意见：">
                        <el-input
                                type="textarea"
                                :rows="6"
                                maxlength="200"
                                show-word-limit
                                v-model="yearReportAudit.remark"
                                style="width: 500px"
                        ></el-input>
                    </el-form-item>
        ...
        <el-button v-if="confirmBtnControl"  type="primary" @click="addYearReportAudit">确 定</el-button>
      </span>
        </el-dialog>
        </div>
        <SolidWQReportSee ref="SonSee"></SolidWQReportSee>
    </div>
</template>

<script>
 import SolidWQReportSee from './solidWasteQuarterReportSee.vue'
    ...
    export default {
        ...
        methods: {
                ...
                this.yearReportAudit.entId =item.entId             
            },
            aduditSee(item,) {
                this.auditDialogVisible = true;
                this.yearReportAudit.entId =item.entId   
                 this.seeEnId=item.entId
                 this.detailSeee=true
                 if(item.status==1 ||item.status==-1 ){
                     this.confirmBtnControl=false

                 }
                    
            },
            //回显查看详情
                   getAuditSolidWasteAnnalsQuarterInfo(item) {
                     
                var _this = this;
                _this.auditStatus = 0;
                let params = new URLSearchParams();
                params.append("entId", _this.seeEnId);
                params.append("declareQuarter", _this.productionNameQuarter);
                params.append("declareYear", _this.search.year);
                params.append("declareType", 2);
                _this.$axios
                    .get("/api/regulatory/commonlysolidwaste/getAuditSolidWasteAnnalsQuarterInfo", {
                        params,
                    })
                    .then(function (res) {
                        if (res.code == 0) {
                    ...
                    .catch((error) => {
                        ...
                        yearClick(tab, event) {
                this.yearReportAudit.declareQuarter = this.productionNameQuarter; 
            
                if(this.detailSeee){
                
    this.getAuditSolidWasteAnnalsQuarterInfo()  
                }  
                   ...
                   addYearReportAudit() {
                var _this = this;
               console.log( _this.yearReportAudit)
                _this.yearReportAudit.declareYear =_this.dialogSearch.reportYear?_this.dialogSearch.reportYear:''
                let para = _this.yearReportAudit
           
                this.$refs["form"].validate((valid) => {
                    if (valid) {
                        _this.$axios
                            .post(
                                "/api/regulatory/commonlysolidwaste/auditSolidWasteAnnalsQuarter",
                               para
                            )
                            .then(function (res) {
                                if (res.code == 0 ) {
                                    ...
                                    _this.$message({
                                        ...
                                        _this.search.year= _this.yearReportAudit.declareYear
                            _this.search.declareQuarter= _this.yearReportAudit.declareQuarter
     ...
     this.confirmBtnControl=true
            ...
            initCountData() {
                this.count.count = 0;
                this.count.commitCount = 0;
                this.count.notCount = 0;
            },
            initMonthList() {
                this.monthList = [];
            ...
            searchEntManagementPlanReport() {
                ...
                _this.initCountData();
                _this.cur_page = 1;
                _this.solidWasteQuarterEntStatistics();
                _this.pageList();
            },
            solidWasteQuarterEntStatistics() {
                var _this = this;
                ...
                params.append('quarter', _this.search.declareQuarter);
                params.append('entName', _this.search.entName);
                _this.$axios.get('/api/regulatory/commonlysolidwaste/solidWasteQuarterEntStatistics', {params}).then(
                    function (res) {
                        if (res.code == 0) {
                            let data = res.data;
                            _this.count.count = data.count;
                            _this.count.commitCount = data.commitCount;
                            _this.count.notCount = data.notCount;
                        }
                    }
                ).catch((error) => {
                    // catch 指请求出错的处理
            ...
            pageList() {
                ...
                params.append('quarter', _this.search.declareQuarter);
                params.append('entName', _this.search.entName);
                _this.$axios.get('/api/regulatory/commonlysolidwaste/listSolidWasteQuarter', {params}).then(
                    function (res) {
                        if (res.code == 0) {
            ...
            // lookEntMonthPeportNew(entId, entName) {
            ...
            // },
                    lookEntMonthPeportNew(entId, entName) {
                       this.$refs.SonSee.dialogVisible=true; 
         	  this.$refs.SonSee.getwastedDetailListZ()
		   this.$refs.SonSee.getStorageList()
		    this.$refs.SonSee.getOperationMethodList()
			 this.$refs.SonSee.getBasicalInfo()
			//   this.$refs.SonSee.getOperationTypeList()
			   this.$refs.SonSee.getFacilityList()			   
			 this.$refs.SonSee.getWasteCategory(entId)
                 this.$refs.SonSee.rowInfoId=entId			
         this.$refs.SonSee.getWholeInfo(entId)
            },
