...
<!--                          <el-table-column
                    ...
                    <el-dialog :title="title" :visible.sync="dialogaddOrEdit" class="dlogBox" :modal-append-to-body='false'
                        ...
                        <el-form label-width="120px" ref="addRegion" :model="form" :rules="regionRules">
                            <el-form-item label="行业名" prop="regionName">
  ...
  dragControllerDiv () {
    ...
    debugger
        ...
        formatterLever(row, column) {
          for (var i = 0; i < this.regionLevelOptions.length; i++) {
            if (row.regionLevel == this.regionLevelOptions[i].warningCode) {
              return this.regionLevelOptions[i].label;
            }
          }
        },
        selectDeptId(val) {
        ...
        init(deptId) {
          deptId = deptId || 0
         let _this = this;
          let url = `/api/permissions/publicData/selectTindutryTree/`
          this.$axios.get(  url,
          ).then(function (res) {
            debugger
            if (res.code === 0) {
        ...
        loadingData(regionCode) {
          ...
          axios.get('/api/permissions/publicData/selectTindutryParent/', {
            params: {
              "value": regionCode
            }
          }).then((response) => {
