          ...
          <tr v-for="(item,i)  in tableDatas">
            ...
            <td style="text-align:center;width: 10%">
              <a
                ...
                @click="jumpAddress('medicalProductionBook',item.entId,item.name,item.regionName)"
              >查看</a>
            </td>
          </tr>
        </tbody>
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    if (to.name == 'medicalProductionBook') {
  ...
  methods: {
    ...
    loadingData() {
	  ...
	  params.append("entSubclass", _this.entSubclass);
