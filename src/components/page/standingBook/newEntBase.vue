    ...
    <div class="main">
        ...
        <div class="main-top">
            <div class="breadcrumb" style="width: 95%">
                ...
                <span class="text-primary"><img src="../../../../static/css/images/pic/logo02.png" style="height: 30px;margin-bottom: -8px;margin-right: 8px"></span><span class="text-primary-font">{{formDate.name}}</span>
            </div>
        </div>
        <div class="main-container " style="width: 100%;height: auto;background-color: #ffffff;">
            <div style="height:auto;width: 100%" class="main-child" id="top-one">
                ...
                <!--================-->
                <div style="margin-right: 20px;margin-left: 56px;margin-top: 20px;padding-bottom: 30px; height: auto;display: flex" v-if="imgList!=null && imgList.length>0">
                    <div v-for="(item ,index) in imgList" style="float: left;height: 280px" :key="index">
                        <div class="img_container" @mouseover="showImg(index)" @mouseout="outImg(index)">
                            <!-- <iframe v-if="item.fileType == 'pdf'" scrolling="no" width="150px" height="200px" :src="imgUrl+item.locationUrl"/> -->

                            <div class="mask" v-if="item.fileType == 'pdf'" >
                                <button @click="openPdf(index,item.fileName,item.pdfUrl,item.fileType)" style="    background: #ffffff;width: 90px;height: 31px; font-weight: bold;">查看详情</button>
                            </div>
                            <div class="mask" v-if="item.fileType == 'doc'" >
                                <button @click="handleDownload(item.locationUrl)" style="    background: #ffffff;width: 90px;height: 31px; font-weight: bold;">下载</button>
                            </div>
                        </div>
                        <div style="text-align: center;width: 150px;margin-top: 20px">{{item.fileName}}</div>
                    </div>
                </div>
                <div style="margin-right: 20px;margin-left: 56px;margin-top: 20px;padding-bottom: 10px; height: auto;display: flex" v-else>
                    暂无
                </div>
                <!--=====================-->
                ...
                <div style="margin-right: 20px;margin-left: 56px;margin-top: 10px;padding-bottom: 10px;">
                    <img :src="imgUrl+imgZhu" style="width: 80%" v-if="imgZhu!=null && imgZhu!=''"/>
                    <span v-else>暂无</span>
                </div>
                <!-- 右侧菜单 -->
                <!--<div id="go_to">-->
...
export default {
  ...
  data() {
    return {
      ...
      formUser: [],
      formUserS: [],
      imgUrl: this.AppConfig.appInfo.IMG_URL,
      divTracing: 1,
      dataIndex: 0,
      imgList: [],
      imgZhu: "",
      ...
      carrentId: this.$route.query.correlationId !=null?this.$route.query.correlationId:'' ,
    };
  },
  mounted() {
    // this.load();
    this.loadingData();
    this.getUserDept();
    console.log(this.$route.query.entKind);
    if(this.carrentId && this.carrentId!=null && this.carrentId!=''){
      this.getImgList();
    }
  },
  methods: {
      backPage(){
        ...
        switch(this.$route.query.entKind) {

          case '产废企业':
            this.$router.push({name:"wasteproDuctionManagement"});
            break;
          case '医疗废物监管':
            this.$router.push({name:"ylenterpriseManagement"});
            break;
          case 'gf':
            this.$router.push({name:"gfWasteEnterprise"});
            break;
          case '智慧监管':
            this.$router.push({name:"enterpriseManagement"});
            break;
          case '运输企业监管':
            this.$router.push({name:"ysenterpriseManagement"});
            break;
          default:
            this.$router.push({name:"businessDuctionManagement"});
        }
      },
    //
    closeDia: function () {
    ...
    load: function () {
      $(document).ready(function () {
        $(".img_container").mouseover(function (e) {
          var $this = $(this);
    ...
    //展示pdf
    openPdf: function (index, name, previewUrl, fileType) {
      this.titleName = name;
    ...
    handleDownload(path) {
      this.$confirm("此操作将下载此文件, 是否继续?", "提示", {
        ...
        params.append("pathUrl", path);
        let locationUrl = path;
        let format = locationUrl.split(".")[1];
        let url = "api/permissions/imageController/downLoad";
        self.$axios
          .post(url, params, { responseType: "blob" })
          .then(function (response) {
            let imgeName = new Date().format("yyyyMMddhhmmss") + "." + format; //下载后图片名
    ...
    getImgList() {
      ...
      let url =
        "/api/regulatory/tEnterpriseTransportPlan/getImgList/" +
        _this.carrentId;
      _this.$nextTick(() => {
        _this.$axios
          .post(url)
          .then((response) => {
            if (response.code == 0) {
              for (let i = 0; i < response.data.length; i++) {
                let obj = new Object();
                if (response.data[i].type == 1 || response.data[i].type == 2) {
                  if (_this.isContains(response.data[i].fileName, ".pdf")) {
                    // let pdf = _this.getPdf(response.data,16);
                    obj.locationUrl = response.data[i].locationUrl;
                    obj.fileName = response.data[i].fileName;
                    obj.pdfUrl = response.data[i].locationUrl;
                    obj.fileType = "pdf";
                    _this.imgList.push(obj);
                  } else if (
                    _this.isContains(response.data[i].fileName, ".doc")
                  ) {
                    // let pdf = _this.getPdf(response.data,16);
                    obj.locationUrl = response.data[i].locationUrl;
                    obj.fileName = response.data[i].fileName;
                    ...
                    _this.isContains(response.data[i].fileName, ".docx")
                  ) {
                    // let pdf = _this.getPdf(response.data,16);
                    obj.locationUrl = response.data[i].locationUrl;
                    obj.fileName = response.data[i].fileName;
